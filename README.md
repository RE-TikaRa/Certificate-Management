...
- **新增**：本地 MCP 服务（stdio/SSE）与可选 Web 控制台（Gradio）
- **新增**：设置页集成 MCP 管理（随软件启动、端口/权限/脱敏配置、Web 账号密码随机/复制、日志入口、Web 依赖一键安装/更新）
- **增强**：MCP 进程管理与日志输出（`logs/mcp_sse.log`、`logs/mcp_web.log`、`logs/mcp_web_install.log`），附件读取支持限额与分段
- **重构**：MCP 相关代码整理到 `src/mcp/`，统一模块命名与脚本入口
- **修复**：导入模板格式与若干设置页布局细节
