...
class StatisticsService:
    ...
    def get_overview(self) -> dict[str, Any]:
        ...
        with self.db.session_scope() as session:
            ...
            result = (
                ...
                .filter(Award.deleted.is_(False))
                .first()
            )
    ...
    def get_group_by_level(self) -> dict[str, int]:
        with self.db.session_scope() as session:
            rows = session.execute(
                select(Award.level, func.count(Award.id)).where(Award.deleted.is_(False)).group_by(Award.level)
            ).all()
    ...
    def get_group_by_rank(self) -> dict[str, int]:
        with self.db.session_scope() as session:
            rows = session.execute(
                select(Award.rank, func.count(Award.id)).where(Award.deleted.is_(False)).group_by(Award.rank)
            ).all()
    ...
    def get_recent_by_month(self, months: int = 6) -> dict[str, int]:
        ...
        months = max(1, months)
        threshold = date.today().replace(day=1) - relativedelta(months=months - 1)
        with self.db.session_scope() as session:
            rows = session.execute(
                select(func.strftime("%Y-%m", Award.award_date), func.count(Award.id))
                .where(Award.deleted.is_(False), Award.award_date >= threshold)
    ...
    def get_award_level_statistics(self) -> dict[str, int]:
        ...
        with self.db.session_scope() as session:
            ...
            level_categories = {"国奖": "国家级", "省奖": "省级", "校奖": "校级"}
            for display_name, level_value in level_categories.items():
                count = session.scalar(
                    select(func.count(Award.id)).where(Award.deleted.is_(False), Award.level == level_value)
                )
                if count:
                    stats[display_name] = int(count)

            rank_categories = {"一等奖": "一等奖", "二等奖": "二等奖", "三等奖": "三等奖", "优秀奖": "优秀奖"}
            for display_name, rank_value in rank_categories.items():
                count = session.scalar(
                    select(func.count(Award.id)).where(Award.deleted.is_(False), Award.rank == rank_value)
                )
                if count:
                    stats[display_name] = int(count)

            extra_ranks = {"一等优秀奖": "一等优秀奖", "二等优秀奖": "二等优秀奖", "三等优秀奖": "三等优秀奖"}
            for display_name, rank_value in extra_ranks.items():
                count = session.scalar(
                    select(func.count(Award.id)).where(Award.deleted.is_(False), Award.rank == rank_value)
                )
                if count:
